            ...
            {activities.map((activity) => (
                <div key = {activity.id} className="px-5 py-10 bg-gray-700 text-white mt-5 flex justify-between rounded-lg">
                    <div className="space-y-2 relative">
                        <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${activity.category === 2 ? "bg-lime-500": "bg-orange-500"}`}>{categoryName(+activity.category)}</p>
                        <p className="text-2xl font-bold pt-5">{activity.name}</p>
                        <p className={`font-black text-3xl ${activity.category === 2 ? "text-lime-500" : "text-orange-500"}`}>
